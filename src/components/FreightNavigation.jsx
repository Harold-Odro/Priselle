...
const NAV_LINKS = [
  { to: '/freight', label: 'Home' },
  { to: '/freight/services', label: 'Services' },
  { to: '/freight/contact', label: 'Contact' }
];
...
export default function FreightNavigation() {
  ...
  const isHomepage = location.pathname === '/freight' || location.pathname === '/freight/';
  ...
  const getNavbarBackground = () => {
    if (isOnDark) {
      return isScrolled ? 'rgba(31, 63, 74, 0.95)' : 'rgba(31, 63, 74, 0.85)';
    }
    return isScrolled ? 'rgba(236, 255, 220, 0.95)' : 'rgba(236, 255, 220, 0.85)';
  };
    ...
    >
      <div className="w-full px-6 sm:px-10 lg:px-16" style={{ height: '70px' }}>
        <div className="flex justify-between items-center h-full">
          {/* Logo - Left Aligned */}
          <Link to="/freight" className="flex-shrink-0 flex items-center group">
            <img
